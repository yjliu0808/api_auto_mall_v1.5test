...
public class BatchWriteToExcel extends BaseLogger {
    ...
    public static void addWriteBackData(int rowNum, int cellNum, int sheetIndex, String content) {
        ...
        logDebug(String.format("📌 收集写回数据 -> sheet: %d, row: %d, col: %d, content: %s",
    ...
    public static void batchWriteToExcel(String excelPath, boolean showLog) {
        if (showLog) {
            logInfo("准备批量写入 Excel，文件路径：" + excelPath);
        }
        ...
        if (writeSize == 0) {
            if (showLog) {
                logInfo("✅ 所有用例执行完毕，批量回写 Excel 成功，共写入 0 条数据。\n");
            }
            return;
        }
        ...
        try (
            ...
            if (showLog) {
                logInfo("✅ 所有用例执行完毕，批量回写 Excel 成功，共写入 " + writeSize + " 条数据。");
            }

        } catch (IOException e) {
            logError("写入 Excel 文件失败: " + excelPath, e);
            throw new RuntimeException("写入 Excel 文件失败: " + excelPath, e);
        } finally {
            if (showLog) {
                logDebug("🧹 清空写回数据列表，当前写回数据如下：");
                for (WriteBackData data : writeBackDataList) {
                    logDebug(String.format("sheet=%d, row=%d, col=%d, content=%s",
